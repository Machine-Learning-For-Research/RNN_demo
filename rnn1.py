...
class RNN:
    ...
    def inference(self):
        ...
        cell = tf.contrib.rnn.BasicLSTMCell(n_inputs)
        init_state = cell.zero_state(self.batch_size, tf.float32)
        # (batch_size, n_steps, n_inputs)
        outputs, _ = tf.nn.dynamic_rnn(cell, x, initial_state=init_state)

        # (n_steps, batch_size, n_inputs)
        outputs = tf.transpose(outputs, [1, 0, 2])
        # n_steps x (batch_size, n_inputs)
        outputs = tf.unstack(outputs)
        # choose last unit output. (batch_size, n_inputs)
        x = outputs[-1]

        W = weight_variables([n_inputs, 10])
